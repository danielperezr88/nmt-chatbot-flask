...
import sys
from os import path, devnull, environ

sys.path.append(path.realpath(path.dirname(__file__)))
sys.path.append(path.join(path.realpath(path.dirname(__file__)), 'nmt'))
...
def do_start_inference(out_dir, hparams):
    ...
    hparams = nmt.create_or_load_hparams(flags.out_dir, tf.contrib.training.HParams(**hparams), flags.hparams_path, save_hparams=True)
...
def infer(question):
    ...
    ans_score = {answer: (answers_rate[i] + scoring.do_scoring(question, answer)) for i, answer in enumerate(answers)}

    scores = list(ans_score.values())
...
if __name__ == "__main__":
    ...
    while True:
        question = input("\n> ")
        answers, answers_rate = inference_internal(question)
        ans_score = {answer: (answers_rate[i] + scoring.do_scoring(question, answer)) for i, answer in
                     enumerate(answers)}
